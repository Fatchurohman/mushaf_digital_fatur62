...
// 2. GENERATOR MAPPING RINCI SAKA DATA API (PARSER MAPPING TANPA PEMOTONGAN TEKS PERMANEN)
function getSurahMappingInfo(surah) {
  ...
  // Yen deskripsi saka API ana, resiki HTML tag tanpa memotong string utuh
  if (surah?.deskripsi) {
...
function renderSurahContent(surah) {
  ...
  // Logika Pemotongan Teks Ringkas & Tombol Selengkapnya
  const maxChars = 200;
  ...
  // Pasang Event Listener ke Tombol Baca Selengkapnya setelah HTML dimasukkan ke DOM
  if (isLong) {
